...
func (r *GroupSigRequest) Validate(db ethdb.Database, msg *Msg) error {
	...
	header := r.Header
	//*** groupSig
	// if hash := types.CalcGroupSigHash(signature); hash != header.MixDigest {
	// 	return errGroupSigHashMismatch
	// }
	addrlist := dbft.AddressList()
	var sigCount int
	for _, sig := range signature {
		if delegateAddr, _ := core.PubKeyrecover(core.EliminateSigningField(header), sig.Sig); delegateAddr != addrlist[int(sig.IIndex)] {
	...
	// 	return fmt.Errorf("no enough signers to pass the groupSig validation")
	// }
	if sigCount < 1 {
		return fmt.Errorf("no enough signers to pass the groupSig validation")
	}
...
func (r *BlockRequest) CanSend(peer *peer) bool {
	return peer.HasBlock(r.Hash, r.Number, true)
}
...
func (r *BlockRequest) Validate(db ethdb.Database, msg *Msg) error {
	...
	//*** groupSig
	// if hash := types.CalcGroupSigHash(body.GroupSig); hash != header.MixDigest {
	// 	return errGroupSigHashMismatch
	// }
	addrlist := dbft.AddressList()
	var sigCount int
